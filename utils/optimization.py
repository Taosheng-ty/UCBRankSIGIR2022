...
# def get_dropout_scores(model, feat,n_samples):
#     score_mat=[]
#     for i in range(n_samples):
#       scores_dropout = model(feat,training=True)[:, 0].numpy()
...
def optimize_policy(model, optimizer,
                    ...
                    print_updates=False,intervene_strategy="Gumbel"):
  ...
  if intervene_strategy=="dropout":
    policy_vali_scores=nn.get_dropout_scores(model, data_vali.feature_matrix,n_eval_samples)
  else:
    policy_vali_scores = model(data_vali.feature_matrix)[:,0].numpy()
  ...
  if intervene_strategy=="dropout":
    dropout_train_scores=nn.get_dropout_scores(model, data_train.feature_matrix,n_grad_samples)
  ...
  for i in range(n_queries*max_epochs):
    ...
    with tf.GradientTape() as tape:
      ...
      tf_scores = model(q_feat,training=True)[:, 0]
      ...
      #                         )[0]
      if intervene_strategy=="dropout":
        ranking_scores=data_train.query_values_from_vector(qid, dropout_train_scores)
      else:
        ranking_scores=scores
      sampled_rankings = pl.get_ranking(
                      ranking_scores,
                      n_grad_samples,
                      cutoff,
    ...
    if (i+1) % (n_queries*early_stop_per_epochs) == 0:
      ...
      metrics =  pl.datasplit_metrics(
                      ...
                      stacked_betas,
                      vali_doc_weights,
                      n_samples=n_eval_samples,intervene_strategy=intervene_strategy
                    )
