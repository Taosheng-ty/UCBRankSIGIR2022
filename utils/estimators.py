...
def prob_per_rank_query(n_samples, cutoff, policy_scores,\
  ...
  n_docs = policy_scores.shape[0]
  q_cutoff = min(cutoff, policy_scores.shape[0])
  if n_docs <= 1:
  ...
  rankings = pl.get_ranking(policy_scores,
                                n_samples,
                                cutoff=q_cutoff,\
  ...
  # np.add.at(freq_per_rank, (rankings, np.arange(q_cutoff)[None,:]), 1)
...
def expected_alpha_beta(
  ...
  for qid in progressbar(range(data_split.num_queries())):
    ...
    q_prob_per_rank = prob_per_rank_query(n_samples,
                                          cutoff,
                                          policy_scores,intervene_strategy=intervene_strategy)
...
def expected_alpha_beta_intervene(
                    data_split,
                    n_samples,
                    alpha,
                    beta,
                    **ranking_param):
  # assert model !=None, "model shouldn't be None"
  n_docs = data_split.num_docs()
  expected_alpha = np.zeros(n_docs, dtype=np.float64)
  expected_beta = np.zeros(n_docs, dtype=np.float64)
  for qid in progressbar(range(data_split.num_queries())):
    ...
    q_beta = data_split.query_values_from_vector(
                                  qid, expected_beta)
    q_feature = data_split.query_values_from_vector(
                                  qid, data_split.feature_matrix)
    ranking_param=ranking_param.copy()
    ranking_param["qid"]=qid
    ranking_param["n_samples"]=n_samples
    ranking_param["cutoff"]=cutoff
    q_prob_per_rank = prob_per_rank_query_intervene(**ranking_param)
...
def prob_per_rank_query_intervene(**ranking_param):
  ...
  policy_scores=select_fn(qid,policy_scores)
  n_docs = policy_scores.shape[0]
  ...
  rankings = pl.get_ranking_intervene(**ranking_param)[0]
