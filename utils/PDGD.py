...
def update_cold(model, optimizer, qid,
          data_split, ranking,
          clicks, scores):

  if not np.any(clicks) or np.all(clicks):
    return
#   print(ranking,scores,"prior")
  mask=scores!=np.NINF
  ori_2_masked=np.ones_like(scores, dtype=np.int32)*np.NINF
#   print(np.where(mask))
#   print(mask,"mask")
  scores=scores[mask]
  n_docs = scores.shape[0]
  
  ori_2_masked[mask]=np.arange(n_docs)
  ranking=ori_2_masked[ranking]
  ranking=ranking.astype(np.int32)
  cutoff = ranking.shape[0]
  cutoff_range = np.arange(cutoff)
#   print(ranking,scores,"post")
  ...
  q_feat = data_split.query_feat(qid)[mask]
