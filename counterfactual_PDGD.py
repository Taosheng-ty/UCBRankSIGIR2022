...
eval_points = np.geomspace(10**2, 4*10**7,
                          n_eval+2,
                          endpoint=True,
                          dtype=np.int32)
update_points = np.geomspace(10**2, 4*10**7,
                            n_updates+2,
                            endpoint=True,
                            dtype=np.int32)[1:-1]
...
logging_metrics = evl.evaluate_policy(
                        logging_model,
                        data.test,
...
for i in range(np.amax(eval_points)):
  ...
  if (n_queries_sampled in eval_points
      or n_queries_sampled in update_points):
  ...
  if n_queries_sampled in eval_points:
    cur_results = {
      ...
      'metrics': cur_metrics,
      'logging policy metrics': logging_metrics,
