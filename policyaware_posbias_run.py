...
# values taken from Addressing Trust Bias for Unbiased Learning-to-Rank, Aman Agarwal et al. 
# pbm_pos_bias = np.array([1., 0.55, 0.38, 0.3, 0.29])
pos_bias = pos_bias = 0.35*1/(np.arange(5)+1.) + 0.65/(1.+0.05*np.arange(5))

est_alpha = pos_bias
est_beta = np.zeros_like(beta)
...
train_clicks = np.zeros((n_updates+1, data.train.num_docs()), dtype=np.int64)
train_displays = np.zeros((n_updates+1, data.train.num_docs()), dtype=np.int64)
train_exp_alpha = np.ones((n_updates+1, data.train.num_docs()))
train_exp_beta = np.ones((n_updates+1, data.train.num_docs()))
...
vali_clicks = np.zeros((n_updates+1, data.validation.num_docs()), dtype=np.int64)
vali_displays = np.zeros((n_updates+1, data.validation.num_docs()), dtype=np.int64)
vali_exp_alpha = np.ones((n_updates+1, data.validation.num_docs()))
vali_exp_beta = np.ones((n_updates+1, data.validation.num_docs()))
...
output = {
  ...
  'run name': 'position-bias policy-aware estimator',
...
(train_exp_alpha[0,:],
 train_exp_beta[0,:]) = est.expected_alpha_beta(
                        data.train,
                        10**3,
                        est_alpha,
                        est_beta,
...
(vali_exp_alpha[0,:],
 vali_exp_beta[0,:]) = est.expected_alpha_beta(
                        data.validation,
                        10**3,
                        est_alpha,
                        est_beta,
...
for n_queries_sampled in eval_points:
  ...
  beta_clip = 0.
  train_doc_weights = est.update_weights_policy_aware(
                            i_update,
                            data.train,
                            train_clicks,
                            train_displays,
                            ...
                            beta_clip=beta_clip)
  vali_doc_weights = est.update_weights_policy_aware(
                            i_update,
                            data.validation,
                            vali_clicks,
                            vali_displays,
  ...
  if n_queries_sampled in update_points:
    ...
    i_update += 1
    (train_exp_alpha[i_update,:],
     train_exp_beta[i_update,:]) = est.expected_alpha_beta(
                            data.train,
                            10**3,
                            est_alpha,
                            est_beta,
    ...
    (vali_exp_alpha[i_update,:],
     vali_exp_beta[i_update,:]) = est.expected_alpha_beta(
                            data.validation,
                            10**3,
                            est_alpha,
                            est_beta,
